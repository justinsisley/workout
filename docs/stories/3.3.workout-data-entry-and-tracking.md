...
2. [ ] Build AMRAP-specific data tracking (AC: 2, 8)
   2.1. [ ] Create `src/components/workout/amrap-data-entry.tsx` component for round-based tracking
   2.2. [ ] Implement round completion tracking instead of individual set tracking
   2.3. [ ] Add partial round completion functionality for incomplete AMRAP rounds
   2.4. [ ] Integrate with existing AMRAP detection using `isAmrapDay` type guard
   2.5. [ ] Add visual indicators for total rounds completed and current round progress

3. [ ] Implement auto-population and smart defaults (AC: 3, 4)
   3.1. [ ] Create server action `src/actions/exercises.ts::getPreviousExerciseData`
   3.2. [ ] Query `exerciseCompletions` collection for user's previous data on same exercise
   3.3. [ ] Pre-fill form inputs with previous workout values (sets, reps, weight, time)
   3.4. [ ] Implement smart suggestion algorithm based on historical performance
   3.5. [ ] Add fallback defaults when no previous data exists

4. [ ] Add data validation and error handling (AC: 6, 10)
   4.1. [ ] Create Zod validation schemas in `src/utils/validation.ts` for workout data
   4.2. [ ] Implement reasonable range validation (weight: 0-1000 lbs, reps: 1-999, etc.)
   4.3. [ ] Add real-time input validation with clear error messaging
   4.4. [ ] Create user-friendly validation feedback for invalid entries
   4.5. [ ] Implement form-level validation before data submission
