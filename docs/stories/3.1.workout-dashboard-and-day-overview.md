...
- [x] Build day overview display component (AC: 1, 4)
  - [x] Create `src/components/workout/day-overview.tsx` component
  - [x] Implement AMRAP day detection using `isAmrapDay` type guard
  - [x] Add specialized AMRAP round-based display with timer countdown
  - [x] Use embedded program structure for single-query data access
  
- [x] Implement exercise list component (AC: 2, 9)
  - [x] Create `src/components/workout/exercise-list.tsx` component
  - [x] Integrate `formatDistance` and `formatDuration` utilities for natural formatting
  - [x] Display sets, reps, rest periods, weight, time, and distance specifications
  - [x] Use `hasDistance` and `hasDuration` type guards for conditional rendering
  
- [x] Add progress indicators component (AC: 3, 8, 10)
  - [x] Create `src/components/workout/progress-indicators.tsx` component
  - [x] Implement round-based progress tracking for AMRAP workouts
  - [x] Show completion status for each exercise using completion tracking
  - [x] Add day-level progress tracking with current position persistence
  
- [x] Build day navigation component (AC: 5)
  - [x] Create `src/components/workout/day-navigation.tsx` component
  - [x] Implement start workout functionality with session initialization
  - [x] Add navigation to individual exercises using Next.js routing
  - [x] Use Zustand workout store for state management
  
- [x] Implement time estimates component (AC: 6)
  - [x] Create `src/components/workout/time-estimates.tsx` component
  - [x] Calculate total day duration from exercise duration specifications
  - [x] Implement AMRAP timer countdown display using JavaScript timers
  - [x] Show time remaining based on current progress through exercises
...
- `src/components/workout/day-overview.tsx` - Day overview display component with AMRAP support
- `src/components/workout/exercise-list.tsx` - Exercise list component with comprehensive formatting and mobile optimization
...
- `src/components/workout/progress-indicators.tsx` - Progress indicators component with AMRAP and session tracking
- `src/components/workout/day-navigation.tsx` - Day navigation component with session management and exercise navigation
...
- `src/components/workout/time-estimates.tsx` - Time estimates component with duration calculations and AMRAP timer countdown
