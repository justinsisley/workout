...
1. [ ] Create exercise detail page component (AC: 1, 2, 11)
   1.1. [ ] Implement `src/app/(frontend)/workout/exercise/[id]/page.tsx` as server component
   1.2. [ ] Add authentication middleware for protected routes
   1.3. [ ] Fetch exercise data using PayloadCMS Local API with populated relationships
   1.4. [ ] Display exercise name, description, and instructions with mobile-optimized typography
   1.5. [ ] Integrate natural format display for duration and distance using existing utilities

2. [ ] Add exercise navigation and routing (AC: 1)
   2.1. [ ] Implement dynamic routing for individual exercise pages
   2.2. [ ] Add navigation between exercises within a workout day
   2.3. [ ] Integrate with existing workout store for exercise progression
   2.4. [ ] Add breadcrumb navigation for current exercise position
   2.5. [ ] Implement back navigation to workout dashboard
