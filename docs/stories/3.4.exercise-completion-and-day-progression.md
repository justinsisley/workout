...
- `src/actions/workouts.ts` - Enhanced with `completeExerciseAndAdvance`, `advanceToNextDay`, `completeAmrapDay` server actions
- `src/stores/workout-store.ts` - Enhanced with day progression, navigation methods, and AMRAP timer management
