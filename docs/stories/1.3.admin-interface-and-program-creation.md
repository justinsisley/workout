...
- [ ] **Time duration input with unit selector** (AC: 6) [ENHANCED from course correction]
  - [x] ~~Add duration field to exercise configuration within embedded days~~ (COMPLETED - needs enhancement)
  - [ ] **ENHANCE**: Replace single duration field with durationValue + durationUnit fields
  - [ ] **ENHANCE**: Add durationValue number field (0-999 range)
  - [ ] **ENHANCE**: Add durationUnit selector (seconds/minutes/hours options)
  - [ ] **ENHANCE**: Implement conditional field logic (unit required if value set, value required if unit set)
  - [ ] **ENHANCE**: Update field layout to accommodate dual fields in exercise row with visual grouping
  - [ ] **ENHANCE**: Add validation for reasonable duration ranges per unit type
  - [ ] **ENHANCE**: Update field descriptions for intuitive usage guidance
...
- **⚠️ COURSE CORRECTION REQUIRED (Sept 9, 2025)** - Single duration field implementation requires enhancement to dual-field approach (durationValue + durationUnit) based on UX friction discovered during first use. Architecture and UX expert reviews completed and approved. Enhancement builds on existing solid foundation without rollback.

### File List

- Modified: `src/payload/collections/programs.ts` - Added duration field to exercise configuration
