...
export function ExerciseVideo({ videoUrl, exerciseTitle, className }: ExerciseVideoProps) {
    ...
    try {
      if (!isFullscreen) {
        if (containerRef.current.requestFullscreen) {
          ...
          ;(
            containerRef.current as HTMLElement & { webkitRequestFullscreen(): void }
    ...
    return (
      <div className={cn('w-full', className)}>
        <Alert>
          ...
          <AlertDescription>No video demonstration available for this exercise.</AlertDescription>
        </Alert>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div className={cn('relative w-full aspect-video', isFullscreen && 'h-full aspect-auto')}>
        <div ref={playerRef} id="youtube-player" className="absolute inset-0 w-full h-full" />
        ...
        <div className="flex items-center justify-between">
          ...
          {exerciseTitle && (
            <div className="flex-1 px-4">
              <p className="text-white text-sm font-medium truncate">{exerciseTitle}</p>
            </div>
          )}
